  ...
  try {
    ...
    if (!checkExist.length  && !mobileExist.length) {
      ...
      if (req.session.userData) {
        // sendVerifyMail(req.body.usrn, req.body.em, userData._id);
        ...
        client.verify.v2.services(authSid)
          ...
          //  req.session.userData.mob = mobile
...
const loadHome = async(req,res,next)=>{
    try {
      ...
      if(req.session.user_id){
        ...
        const couponUpdate = await Coupon.findOneAndUpdate({used:2},{$set:{status:'Inactive'}})
        ...
        if(cartData && wishlistData ){
           ...
           res.render("home",{userData,categoryData,productData,cartCount,cartData,wishlistData,wishlistCount,banner})
        }else if(cartData) {
          cartCount = cartData.products.length;
          res.render("home",{userData,categoryData,productData,cartCount,cartData,wishlistData,wishlistCount,banner})
...
const ResetPassword = async(req,res,next)=>{
  ...
  const updatedData1 = await  User.findByIdAndUpdate({_id:user_id},{$set:{password:secure_password,token:''}})
    res.redirect('/login');
  } catch (error) {
    console.log(error.message);
      next(error.message);
...
const check = async (req,res,next)=>{
  ...
  //  const otp =concatenate(req.body.otp1,req.body.otp2,req.body.otp3,req.body.otp4,req.body.otp5,req.body.otp6)
          ...
          if(req.session.userData){
            ...
            (async function update(){
              // const updatedData =await User.updateOne({mobile:number},{$set:{ is_verified:1}})
               const spassword = await securePassword(req.session.userData.psw);
                      const user = new User({
...
const loadShop = async (req, res,next) => {
  try {      
      ...
      const limit = 5;
      let productData = await Product.find({list:0}).populate('category').limit(limit * 1)
      .skip((page - 1) * limit).exec();
      let brand = await Product.find().distinct('brand');
      
      const count = await Product.find({list:0}).populate('category').countDocuments()
      const categoryData = await Category.find({list:0});
    ...
    if(req.session.user_id){  
      ...
      if(req.query.id){
        ...
        const productData =await Product.find({category:category,list:0}).limit(limit * 1)
        .skip((page - 1) * limit).exec();
        const count = await Product.find({category:category,list:0}).countDocuments()
        ...
        if(cartData && wishlistData){
          ...
          res.render("shop",{userData,categoryData,productData,cartCount,wishlistData,wishlistCount,brand,totalPages:Math.ceil(count/limit),currentPage:page})
      ...
      if(req.query.id){
        ...
        const count = await Product.find({category:category,list:0}).countDocuments()
        const categoryData = await Category.find({list:0}); 
        res.render("shop",{categoryData,productData,brand,wishlistData:0,totalPages:Math.ceil(count/limit),currentPage:page})
      }else{
  ...
  try {
    ...
    if(req.session.user_id){
      ...
      cartCount = cartDatas.products.length;
           ...
           res.render('detail',{userData,productData,cartData,categoryData,cartCount,wishlistCount})
      }else if(cartDatas){
         cartCount = cartDatas.products.length;
...
const addtoWishlist = async(req,res,next)=>{
  try {
    if(req.session.user_id){
      ...
      if(wishlistData){
        let wishlistExists = await Wishlist.findOne({userId:userData.id,products:{$elemMatch:{$in:id}}})
        if(wishlistExists === null){
          const updateWishlist = await Wishlist.findOneAndUpdate({userId:userData},{$push:{products:id}})
  ...
  try {
    ...
    if(req.session.user_id){
      ...
      if(wishlist && cartDatas){
        ...
        cartCount = cartDatas.products.length;
         res.render("wishlist",{userData,categoryData,productData,wishlistData,wishlistCount,cartData,cartCount})
      }else if(wishlist){
        const data = wishlist.products;
        wishlistData = await Product.find({_id:{$in:data}})
        wishlistCount = wishlist.products.length;
  ...
  try {
    ...
    if(req.session.user_id){
      ...
      if(cartData && wishlistData){
        cartCount =cartData.products.length;
        wishlistCount = wishlistData.products.length;
         res.render("contact",{userData,categoryData,productData,cartCount,wishlistCount})
...
//putting products to database
      ...
      if(cartData.length > 0){
          let duplicateData = await Cart.findOne({userId:userData.id,products:{$elemMatch:{productId:id}}})
        ...
        const updateCartData = await Cart.findOneAndUpdate({userId:userData},{$push:{products:{...productData1}}})
        res.redirect('/shop')
          }else{
           const dup = await Cart.findOneAndUpdate({userId:req.session.user_id,"products.productId":id},
            {$inc:{'products.$.quantity':1}});

            let data = await Cart.findOne({userId:req.session.user_id,"products.productId": id})
            let quantity = data.products.find((pro)=>pro.productId == id).quantity
            const price = quantity * productData.offerPrice;
            const updateCart = await Cart.findOneAndUpdate({userId:req.session.user_id,"products:productId":id},
      ...
      res.redirect('/shop')
        // res.render("cart",{userData,categoryData,productData})
}else{
      // const categoryData1 = await Category.find({},{_id:1});
      // const values = categoryData1.map(category=>category._id);
      // const productData = await Product.find({category:{$in:values}}).populate('category');
      // const categoryData = await Category.find({__v:0});

      // res.render("cart",{categoryData,productData})
      res.redirect('/')
    }
  } catch (error) {
    console.log(error.message);
...
const increment = async(req,res,next)=>{
  ...
  try {
    ...
    let updatedData = await Cart.findOneAndUpdate({userId:req.session.user_id,"products.productId":id},
    {$inc:{'products.$.quantity':1 ,"products.$.price":productData.offerPrice}})

      let cartData = await Cart.findOne({userId:req.session.user_id,"products.productId":id})
...
const decrement = async(req,res,next)=>{
  ...
  try {
    ...
    let updateData =  await Cart.findOneAndUpdate({userId:req.session.user_id,"products.productId":id},
      ...
      cartData = await Cart.findOne({userId:req.session.user_id,"products.productId":id})
  ...
  try {
    if(req.session.user_id){
      ...
      const userData = await User.findById({_id:req.session.user_id});
       ...
       cartData = await Cart.findOne({userId:userData.id}).populate("products.productId");
        wishlistData = await Wishlist.findOne({userId:userData.id});
      ...
      if(cartData && wishlistData){
       cartCount = cartData.products.length;
       wishlistCount =wishlistData.products.length;
        res.render('cart',{data,userData,productData,cartData,categoryData,cartCount,wishlistCount})
      }else if(wishlistData){
        console.log("2work");
       
        wishlistCount =wishlistData.products.length;
        res.render('cart',{userData,productData,cartData,categoryData,cartCount,wishlistCount,data}) 
      }else if(cartData){
        console.log("3work");
...
const removeCart = async(req,res,next)=>{
  try {
      const id = req.query.id;
      const deleteData = await Cart.updateOne({userId : req.session.user_id},{$pull:{products:{productId:id}}})
      res.redirect('/cart')
  } catch (error) {
    console.log(error.message);
      next(error.message);
...
const removeWishlist = async(req,res,next)=>{
  try {
    if(req.query.productId){
      ...
      const deleteData= await Wishlist.updateOne({userId:req.session.user_id},{$pull:{products:id}})
      res.redirect('/wishlist')
      res.json(response)
...
const loadCheckout = async(req,res,next)=>{
  try {
    if(req.session.user_id){
      ...
      res.render("checkout",{userData,categoryData,productData,cartCount,wishlistCount,cartProducts,data,cartData,couponData})
      ...
      // const categoryData = await Category.find({__v:0});
      res.redirect('/shop')
      // res.render("checkout",{categoryData,productData,userData,wishlistCount,wishlistData,cartCount,cartData})
    }
  } catch (error) {
    console.log(error.message);
      next(error.message);
...
const loadOrderPage = async(req,res,next)=>{
    try {
      ...
      if(req.session.user_id){
        ...
        const addressExist = await User.find({ address: { $elemMatch: {name:req.body.name,
          mobile : req.body.mobile, city: req.body.city, state: req.body.state,house: req.body.house,pinCode: req.body.pinCode, district: req.body.district, country: req.body.country,street:req.body.street } } });
        ...
        if(couponData){
         totals = total - (total * (couponData.discount/100))
          if((total*(couponData.discount/100)) >= couponData.maxRedeemAmount){
       ...
       const deliveryDate1 = deliveryDate.toString().slice(0,15);
        ...
        // res.render('orderPage',{userData,productData,orderData,addressArray,data});
        // res.redirect('/orderPage')
          const orderRecipt = await Order.findOne({userId:req.session.user_id}).sort({createdAt:-1}).limit(1);
...
const  verifyPayment = async(req,res)=>{
  ...
  if (generated_signature == razorpayS) {
    ...
    const orderData = await Order.findOne({}).sort({createdAt:-1}).limit(1);
    
    
    const updateData = await Order.findByIdAndUpdate({_id:orderData._id},{$set:{paymentStatus:"Completed"}})
  }else{
    const updateorder = await Order.deleteOne({ _id: ObjectId(orderData._id) });
...
const orderPageView = async(req,res)=>{
  try {
    if(req.session.user_id && req.session.orderId ){
      ...
      const orderData = await Order.findOne({userId:userData._id}).sort({createdAt:-1}).populate('products.productId');
      ...
      for(const products of orderData.products){
       ...
       const update = await Product.updateOne({_id:products.productId},{$set:{stocks:updatedStock}})
       }
      ...
      const deleteData = await Cart.updateOne({userId:orderData.userId},{"$unset":{"products":""}})
    ...
    if(req.session.user_id){
      ...
      if(cartData && wishlistData){
        ...
        wishlistCount = wishlistData.products.length;
         ...
         res.render("profile",{userData,categoryData,productData,cartCount,wishlistCount})
      ...
      // res.render("profile",{categoryData,productData,userData})
      res.redirect('/')
    }
    ...
    if(req.session.user_id){
      ...
      if(cartData && wishlistData){
        ...
        wishlistCount = wishlistData.products.length;
         ...
         res.render("editProfile",{userData,categoryData,productData,cartCount,wishlistCount})
...
const updateProfile = async(req,res,next)=>{
  try {
    if(req.session.user_id){
      ...
      await User.findByIdAndUpdate({_id:req.session.user_id},{$set:{name:name,email:email,mobile:mobile}});
...
const resendOtp = async(req,res,next)=>{
  try {
    ...
    console.log(req.session.userData,"check");


    const number = req.session.userData.mob
  
        client.verify.v2.services('VAb90a939e06b862aff9d17e697952cadb')
        function sendTextMessage(){
          client.verify.v2.services('VAb90a939e06b862aff9d17e697952cadb')
          .verifications
          .create({to:'+91'+number, channel: 'sms'})
  ...
  try {
    ...
    if(cartData && wishlistData){
      ...
      wishlistCount = wishlistData.products.length;
       ...
       res.render("editAddress",{userData,categoryData,productData,cartCount,wishlistCount,addressData})
      ...
      cartCount =cartData.products.length;
       ...
       res.render("editAddress",{userData,categoryData,productData,cartCount,wishlistCount,orderData,addressData})
...
const deleteAddress = async(req,res,next)=>{
  try {
    if(req.session.user_id){
      ...
      const deleteAddress = await User.findOneAndUpdate({_id:ObjectId(req.session.user_id)},{$pull:{address:{_id:id}}})
...
const pushAddress = async(req,res,next)=>{
  ...
  const updateAddress = await User.findByIdAndUpdate({_id:req.session.user_id},{$push:{address:{...addressData},}})
...
const updateAddress = async(req,res,next)=>{
    try {
      if(req.session.user_id){
       ...
       const updateAddress = await User.findByIdAndUpdate({_id:req.session.user_id,'address._id':id},{$set: {'address.$':addressData}})
    ...
    if(req.session.user_id){
      ...
      if(cartData && wishlistData){
        ...
        wishlistCount = wishlistData.products.length;
         ...
         res.render("manageAddress",{userData,categoryData,productData,cartCount,wishlistCount,orderData})
...
const orderlist = async(req,res,next)=>{
  try {
      ...
      const orderData = await Order.find({userId:req.session.user_id}).populate('products.productId').sort({createdAt:-1});
      ...
      if(cartData && wishlistData){
        ...
        wishlistCount = wishlistData.products.length;
         ...
         res.render("orderlist",{userData,categoryData,productData,cartCount,wishlistCount,orderData})
...
const orderView = async(req,res,next)=>{
  try {
    ...
    cartData = await Cart.findOne({userId:userData.id})
     wishlistData = await Wishlist.findOne({userId:userData.id});
     const id = req.query.id;
     const orderData = await Order.findById({_id:id}).populate('products.productId').populate('address.address'); 
     const add=orderData.address
    ...
    if(cartData && wishlistData){
      ...
      wishlistCount = wishlistData.products.length;
       ...
       res.render("orderView",{userData,categoryData,productData,cartCount,wishlistCount,orderData,address})
...
try {
      if(req.session.user_id){
        ...
        if(orderData.paymentMethod == 'COD'){
          const orderUpdate = await Order.findByIdAndUpdate({_id:req.query.id},{$set:{orderStatus:'Cancelled',paymentStatus:"Cancelled"}});
        }else{
           const orderUpdate = await Order.findByIdAndUpdate({_id:req.query.id},{$set:{orderStatus:'Cancelled',paymentStatus:'Refunded'}});
        }
...
if (existCoupon) {
  ...
  const userCheck = await Coupon.findOne({code: coupon, claimedBy: req.session.user_id});
  const today =new Date()
        today.setHours(0, 0, 0, 0);
  const exp = existCoupon.expiryDate
        exp.setHours(0, 0, 0, 0);
  if (userCheck) {
    console.log(`${userCheck} already claimed by user`);
    res.json({message:"already used"});
  } else if(exp.getTime() <= today.getTime()) {

    res.json({message:'coupon expired'})
    const updatedCoupon = await Coupon.findOneAndUpdate({code:coupon},{$set:{status:'expired'}});
  }else{
  ...
  if (existCoupon.minPurchaseAmount <= sum) {
    ...
    if (discountAmount <= existCoupon.maxRedeemAmount) {
      ...
      const discountedPrice = Math.round(amount - existCoupon.maxRedeemAmount);
      ...
      res.json({success:true,discountedPrice, discountAmount: existCoupon.maxRedeemAmount });
    }
  } else {
...
const filterBrand = async(req,res,next)=>{
  try {
    ...
    let productData = await Product.find({brand:{$in:selectedBrands},list:0}).populate('category').limit(limit * 1)
    .skip((page - 1) * limit).exec();
